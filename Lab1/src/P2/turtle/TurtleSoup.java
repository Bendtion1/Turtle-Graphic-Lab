...
public class TurtleSoup {
	...
	public static double calculateRegularPolygonAngle(int sides) {
		...
		return (double) Math.round(angle * 100) / 100.0;
	...
	public static double calculateHeadingToPoint(double currentHeading, int currentX, int currentY, int targetX,
			int targetY) {
		// throw new RuntimeException("implement me!");
		double x = Math.abs(targetX - currentX);
		double y = Math.abs(targetY - currentY);
		...
		if (angle - currentHeading >= 360)
			return Math.round((angle - currentHeading - 360) * 100) / 100.0;
		return Math.round((angle - currentHeading) * 100) / 100.0;
	}
	...
	public static List<Double> calculateHeadings(List<Integer> xCoords, List<Integer> yCoords) {
		...
		List<Double> currentHeading = new ArrayList<>();
		currentHeading.add(0.0);
		for (int i = 0; i < xCoords.size() - 1; i++) {
			System.out.println(xCoords.get(i));
			double angle = calculateHeadingToPoint(currentHeading.get(i), xCoords.get(i), yCoords.get(i),
					xCoords.get(i + 1), yCoords.get(i + 1));
			vector.add(angle);
			currentHeading.add((currentHeading.get(i) + angle) % 360);
		}
		return vector;
	}
...
/*	public static void drawPersonalArt(Turtle turtle) {
	...
	public static void drawPersonalArt(Turtle turtle) {
		...
		for(int i=0; i<100;i++)
		{
		drawRegularPolygon(turtle, 5, (2*i+7));
		turtle.turn(8);
		}
	}
